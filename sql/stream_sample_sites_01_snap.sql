...
-- from https://github.com/smnorris/fwapg/blob/main/extras/channel_width/sql/fiss_stream_sample_sites_events.sql

DROP TABLE IF EXISTS fiss_stream_sample_sites;

CREATE TABLE fiss_stream_sample_sites AS
...
(
  ...
  INNER JOIN whse_basemapping.fwa_watershed_groups_poly wsd
  ON ST_Intersects(pt.geom, wsd.geom)
  LEFT OUTER JOIN fiss_stream_sample_sites e
...
INSERT INTO fiss_stream_sample_sites
(
  stream_sample_site_id,
  linear_feature_id,
  blue_line_key,
...
create index on fiss_stream_sample_sites (blue_line_key);